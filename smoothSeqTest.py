...
while testedActionsCount < 1000:
  ...
  if ImgToObj.check_seq_action_intersect(empty_seq_data, stride, goal_type,np.squeeze(random_action[0:3])):
    sim_result = simulator.simulate_action(0, np.squeeze(random_action[:,0:3]), need_images=True, stride=1)
    if not sim_result.status.is_invalid():
      testedActionsCount += 1
      seq_data = ImgToObj.getObjectAndGoalSequence(sim_result.images)
      #totalDist = float("inf")
      #for object_data, goal_data in zip(seq_data['object'], seq_data['goal']):
      #  totalDist = min(totalDist ,np.linalg.norm(object_data['centroid'] - goal_data['centroid']))
